...
| Parameter       | Default     | Description |
|-----------------|-------------|-------------|
| `nlow`          | 1           | Minimum mode index. Physical wavenumber k = n × 2π/L. |
| `nhigh`         | 3           | Maximum mode index. Modes with nlow² ≤ nkx²+nky²+nkz² ≤ nhigh² are included. |
| `spectrum`      | `parabolic` | Spectrum form: `band`, `parabolic`, or `power_law`. The default peaks at k = 2. |
...
| `iseed`         | 210989      | Integer RNG seed. Positive and negative values with the same magnitude initialise the same sequence. Different seeds give independent realisations with the same statistical properties. |
