...
SpectralICGenerator::SpectralICGenerator(MeshBlockPack *pmbp, ParameterInput *pin,
  ...
  // k_phys = n * 2π / L.  The default band [1,3] peaks at n=2 when using
  ...
  rstate.idum = (iseed <= 0) ? iseed : -iseed;
...
// heFFTe path: MPI-distributed r2c IFFT
#if HEFFTE_ENABLED
  {
    ...
    // Slab rank builds send lists for each world rank.  A grid sample can be
    // needed by more than one meshblock when it sits on a meshblock upper face
    // in x/y/z, so send it to every meshblock owner whose extended local
    // [is:ie+1, js:je+1, ks:ke+1] range contains the sample.
    struct TargetSample {
      int gid, li, lj, lk;
      Real val_ax, val_ay, val_az;
    };
    std::vector<std::vector<TargetSample>> send_lists(world_nranks);

    if (participates) {
      auto add_targets_1d = [](int g, int nmb_dim, int ncell_mb,
                               std::vector<int> &targets) {
        targets.clear();
        const int primary = g / ncell_mb;
        targets.push_back(primary);
        if ((g % ncell_mb) == 0) {
          targets.push_back((primary + nmb_dim - 1) % nmb_dim);
        }
      };
      std::vector<int> lx_targets, ly_targets, lz_targets;
      for (int ix = 0; ix < local_nx; ++ix) {
        const int gx = slab_x0 + ix;
        add_targets_1d(gx, nmb_x, nx_mb, lx_targets);
        for (int gy = 0; gy < ny; ++gy) {
          add_targets_1d(gy, nmb_y, ny_mb, ly_targets);
          for (int gz = 0; gz < nz; ++gz) {
            const int64_t id = (static_cast<int64_t>(ix) * ny + gy) * nz + gz;
            add_targets_1d(gz, nmb_z, nz_mb, lz_targets);
            for (int lx1 : lx_targets) {
              for (int lx2 : ly_targets) {
                for (int lx3 : lz_targets) {
                  const int flat = (lx3 * nmb_y + lx2) * nmb_x + lx1;
                  const int gid = lloc_to_gid[flat];
                  const int tgt = mb_owner[gid];
                  int li = gx - lx1*nx_mb;
                  int lj = gy - lx2*ny_mb;
                  int lk = gz - lx3*nz_mb;
                  if (li < 0) li += nx;
                  if (lj < 0) lj += ny;
                  if (lk < 0) lk += nz;
                  send_lists[tgt].push_back({gid, li, lj, lk,
    ...
    // Copy received samples into host mirrors of ax/ay/az, then deep_copy to device.
    ...
    // Build global meshblock id -> local meshblock index map.
    ...
    for (const auto &s : recv_flat) {
      const int m = local_mb_lmap[s.gid];
      if (m < 0) continue;
      const int i = is + s.li;
      const int j = js + s.lj;
      const int k = ks + s.lk;
